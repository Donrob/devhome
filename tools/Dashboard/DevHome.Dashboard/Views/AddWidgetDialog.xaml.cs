...
public sealed partial class AddWidgetDialog : ContentDialog
{
    ...
    private async Task FillAvailableWidgetsAsync()
    {
        ...
        var unsafeWidgetDefinitions = await Task.Run(() => catalog!.GetWidgetDefinitions());
        var comSafeWidgetDefinitions = unsafeWidgetDefinitions.Select(x => new ComSafeWidgetDefinition(x)).OrderBy(x => x.DisplayTitle);
        ...
        var unsafeCurrentlyPinnedWidgets = await Task.Run(() => host.GetWidgets());
